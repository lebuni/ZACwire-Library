...
class ZACwire {
  ...
  public:
	...
	float getTemp() {	    					//gives back temperature in °C
		...
		static byte newBitWindow = _defaultBitWindow;
		...
		if (BitCounter == 19) newBitWindow = ((ByteTime << 5) + (ByteTime << 4) + ByteTime >> 9) + (range >> 1);	//divide by 10.5
	...
	static void IRAM_ATTR read(void *arg) {
  	...
  	static void read() {						//gets called with every rising edge
		...
		if (++BitCounter > 4) {					//first 4 bits always =0
			...
			if (BitCounter == 10) {   			//after stop bit
				ByteTime = microtime - ByteTime;
				ByteNr = 1;
				rawTemp[1][backUP] = 0;
			}
			else if (deltaTime >> 10) {		  	//true at start bit
			...
			rawTemp[ByteNr][backUP] <<= 1;      
  			...
  			else if (deltaTime < bitWindow - (range >> (BitCounter==11)) || rawTemp[ByteNr][backUP] & 2) rawTemp[ByteNr][backUP] |= 1;	//Logic 1
			deltaTime = microtime;
		}
  	}
...
template<uint8_t pin>
volatile byte ZACwire<pin>::BitCounter = 0;
template<uint8_t pin>
